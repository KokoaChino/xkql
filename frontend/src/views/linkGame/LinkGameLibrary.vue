...
import { _POST, post } from "@/net/index.js";
...
const init = async (size) => {
    if (is_gain.value)
        G.value = await post("/api/link-game/get-library", size);
    g.value = cloneDeep(G.value[0]);
    p.value = 0;
    vis.value = await post("/api/link-game/create-visited", size);
    path.value = [];
    cache.value = new Map();
    ckey.value = 1;
    color.value = 1;
...
const submit = async () => {
    ...
    await init(n.value);
    is_gain.value = true;
    _POST("/api/link-game/set-library",
