...
const init = async () => {
    G.value = await post("/link-game/get-library", n.value);
    if (G.value.length <= 1) return;
    if (p.value !== 0) prev_p.value = p.value;
    p.value = await post("/link-game/get-random", [G.value.length, prev_p.value])
    await reset();
...
const reset = async () => {
    ...
    vis.value = await post("/link-game/create-visited", n.value);
    path.value = [];
    cache.value = new Map();
    ckey.value = 1;
}
        ...
        if (g.value[xi][yi][0] === col && g.value[xi][yi][1] === 0) { // 路径终点
            if (directions.some(([dx, dy]) => xi === dx && yi === dy)) { // 相邻
                ...
                if (--cnt.value === 0) { // 游戏结束
                    ...
                    clearTimeout(timerId.value); // 取消定时器
                    await post("/link-game/historical-record/update", [store.auth.user.username, n.value, formatSeconds(seconds.value)])
                }
            }
        }
        return;
